...
public class GoodsServlet10014 extends HttpServlet {
	private static final long serialVersionUID = 1L;
 
    public GoodsServlet10014() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String name = s1.getAttribute("keywords4").toString().trim();	
		...
		if (name==null || name.isEmpty()) {	
		 ...
		 //  sql="select top " + num + " 图书编码,图书名称,作者,价格,图片地址 from 图书数据表  where 图书名称 like '%" + name + "%' or 作者 like '%" + name + "%'";
			 sql="select * from 图书详情表   where 类型 like '%" + name + "%'";
		}
		...
		try {
			while(rs.next()){
				...
				goods.setGoodsImageAddress4(rs.getString("大图片地址"));
		...
		if (name==null || name.isEmpty()) {	
		   ...
		   request.setAttribute("searchName", "全部图书");
			...
			sql="select  count(*) from 图书详情表  where 类型   like '%" + name + "%'";
		...
		request.getRequestDispatcher("show.jsp").forward(request, response);		//重定向页面
	}	
}
